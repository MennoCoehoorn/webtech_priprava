...
function showSearchPop(){
	...
	if (state==="none"){
		...
		searchBar.style.display="inline-block"
		document.querySelector('#searchBtn2').style.display="inline-block";
...
function filter_load(url) {
	//var itemValue = localStorage.getItem("collection");
	var itemValue=getParameterByName('collection',url)
	if (itemValue) {
	...
	//itemValue = localStorage.getItem("color_name")
	itemValue=getParameterByName('color_name',url)
	if (itemValue) {
		itemValue=itemValue.split(',');
	...
	//itemValue = localStorage.getItem("size_name")
	itemValue=getParameterByName('size_name',url)
	if (itemValue) {
		itemValue=itemValue.split(',');
...
function filter_to_query(curr_url,full_url){
	...
	var search_string=getParameterByName('query',full_url);
	console.log('Halo '+search_string);
	var query_string='';
	if(search_string!=null){
		query_string=query_string+'?query='+search_string;
	}
	if (curr_url.includes('?')===false && query_string.includes('?')===false){
