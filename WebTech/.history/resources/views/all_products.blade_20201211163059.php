...
<!DOCTYPE html>
 ...
 @endsection
 	...
 	<section class="container" id="main" >
		...
		@if (count($produkty)>0)
		@if (request()->category)
 		<h1 class="capitalize">{{$produkty[0]->category}}</h1>
		@else
		<h1 class="">Výsledky vyhľadávania</h1>
		@endif
		@else
		<h1 class="">Žiadne produkty</h1>
		@endif
 		...
 		<section class="filters row">
 			<section class="col-8 left_filter">
 				...
 				<button class="filter_button" onclick=radio_load() data-toggle="dropdown" id="sortDropdown" aria-haspopup="true" aria-expanded="false">
				 	<p class="d-none d-sm-inline-block">Zoradiť</p>
 			...
 			<section class="col-4 right_filter">
 				<button class="filter_button" onclick="filter_load('{{url()->full()}}')">
				 <p class="d-none d-sm-inline-block">Filtrovať</p>
 				...
 				<aside id="mySidenav" class="sidenav">
 					...
 					<section>
						...
						@foreach($allsizes as $size)
							<label><input type="checkbox" value='{{$size->size_name}}' name="size_name">{{$size->size_name}}</label>
							<input type="checkbox" value='{{$color->color_name}}' name="color_name" >{{$size->size_name}}</label>
						@endforeach
 					...
 					<div class="row">
 						...
 						<div class="col text-center" onclick="filter_to_query('{{url()->current()}}','{{url()->full()}}')" >
		 ...
		 @foreach($produkty as $prod)
  		 ...
  		 {{--<p> {{$prod->name}}</p>--}}
			<section class="col-lg-3 col-md-4 col-sm-6 col-xs-12 mb-3 d-flex align-items-stretch">
				<a href="{{route('productdetail', ['gender' => $prod->sex,
																	'category'=>$prod->category,
																	'id'=> $prod->id])}}" >
					<div id="karticka" class="card h-100 w-100">

						<div class="view overlay">
							<picture>
								<source srcset="/{{$prod->picture_path['picture_path_lg']}}"
									media="(min-width: 1200px)">
								<source srcset="/{{$prod->picture_path['picture_path_md']}}"
									media="(min-width: 768px)">
								<source srcset="/{{$prod->picture_path['picture_path_s']}}"
									media="(min-width: 576px)">
								<source srcset="/{{$prod->picture_path['picture_path_xs']}}"
									media="(max-width: 575px)">


								<img src="/{{$prod->picture_path['picture_path_md']}}" width=100%  />

							</picture>
							{{--<img src="/{{$prod->picture_path['picture_path_md']}}" width=100% alt="foto šiat" />--}}						
						</div>
						...
						<div class="card-body text-left">
							...
							{{$prod->price}}€
								...
								<h5 >
									<!--<a href="{{route('productdetail', ['gender' => $prod->sex,
																	'category'=>$prod->category,
																	'id'=> $prod->id])}}" class="stretched-link"> -->
								...
								<div class="color_swatch align-items-left">
									...
									@foreach(array_unique(explode(',',$prod->col)) as $color)
	...
	@endforeach
		...
		<section id="pagin" class="row justify-content-center">
