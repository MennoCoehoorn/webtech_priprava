...
class StockController extends Controller
{
    ...
    public function list($page) {  
        ...
        if ($rowsPerPage == 0) {
            ...
            $products = DB::table('products_sizes_colors')->select('products_sizes_colors.id as idecko','products_sizes_colors.nam', 'products_sizes_colors.*', 'sizes.size_name', 'colors.color_name')
                ->join('products','products_sizes_colors.product_id','products.id')
                ->join('sizes','products_sizes_colors.size_id','sizes.id')
                ->join('colors','products_sizes_colors.color_id','colors.id')
                ->orderBy($sortBy, $descending)
