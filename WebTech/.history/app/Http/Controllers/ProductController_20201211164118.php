...
class ProductController extends Controller
{
    ...
    public function destroy(Product $product)
    {
        ...
        error_log($pics);
        $deleted=Picture::where([['product_id',$id]])->delete();
        error_log($deleted);
        foreach ($pics as $pic){
            if (file_exists($pic->picture_path)) 
               {
                 unlink($pic->picture_path);
                  error_log("File Successfully Delete."); 
              }
            else {
                error_log("NENI");
            }
    ...
    public function search(){
        ...
        $allcolors=Product::select('colors.color_name')
        ->join('products_sizes_colors','products.id','=','products_sizes_colors.product_id')
        ->join('colors','products_sizes_colors.color_id','=','colors.id')
        ->where([['products.name','like', '%'.$searchtext.'%']])
        ->orWhere([['products.description','like', '%'.$searchtext.'%']])
        ->groupBy('colors.color_name')->get();

        $allsizes=Product::select('sizes.size_name')
        ->join('products_sizes_colors','products.id','=','products_sizes_colors.product_id')
        ->join('sizes','products_sizes_colors.size_id','=','sizes.id')
        ->where([['products.name','like', '%'.$searchtext.'%']])
        ->orWhere([['products.description','like', '%'.$searchtext.'%']])
        ->groupBy('sizes.size_name')
        ->orderBy('sizes.id')->get();


        $allcollections=Product::select('collection')
        ->where([['products.name','like', '%'.$searchtext.'%']])
        ->orWhere([['products.description','like', '%'.$searchtext.'%']])
        ->groupBy('collection')->get();
        ...
        return view('all_products',['produkty'=>$products_get, 'price'=>$price, 'allsizes'=>$allsizes,'allcolors'=>$allcolors, 'allcollections'=>$allcollections]);
    }
    ...
    public function index($gender,$category){
        ...
        ->join('products_sizes_colors','products.id','=','products_sizes_colors.product_id')
        ->join('colors','products_sizes_colors.color_id','=','colors.id')
        ->where([['sex',$gender],['category',$category]])
        ->groupBy('colors.color_name')->get();

        $allsizes=Product::select('sizes.size_name')
        ->join('products_sizes_colors','products.id','=','products_sizes_colors.product_id')
        ->join('colors','products_sizes_colors.size_id','=','sizes.id')
        ->where([['sex',$gender],['category',$category]])
        ->groupBy('sizes.size_name')
        ->orderBy('sizes.id')->get();
        ...
        return view('all_products',['produkty'=>$products_get, 'price'=>$price, 'allcolors'=>$allcolors,'allcolors'=>$allcolors, 'allcollections'=>$allcollections]);
    }
